...
from DeviceControl.idevice.utils.EnergyCollector import EnergyCollector
...
class EnergySampler:
    ...
    def _worker(self):
        with DvtSecureSocketProxyService(lockdown=self.service_provider) as dvt:
            with EnergyCollector(dvt, self.pid) as collector:
    ...
    def start(self):
        if self.running:
            print("📡 EnergySamplerSampler 已经运行")
            return
        self.running = True
